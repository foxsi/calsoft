...
; Last Mod 	: 31 Oct 2017 14:27

;copied from draw_64ecquad.pro to include cr line
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
PRO DRAW_64ECQUAD_CR, PEAKS, ASIC, MAKEPLOT, XRANGE = XRANGE, YRANGE = YRANGE
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
 ...
 nl=dblarr(9,64,4)
 quad_residual=dblarr(9,64,4)
 quad_ratio=dblarr(9,64,4)
 fitted_peak=dblarr(9,64,4)
 ...
 for asic=0,3 do begin
	  ...
	  !Y.MARGIN=!Y.MARGIN/1.
	  set_plot,'PS'
	  fname='Gaincal_quadfit_asic'+strtrim(string(asic),2)+'cr.eps'
	 ...
	 fitted_peak[0:n_elements(polyfit)-1,ch,asic] = polyfit
	 ...
	 ;if(asic le 1) then xrange=[0,200] else xrange=[0,350]
	 plot, peaks[2:xr, ch, asic,0], peaks[2:xr, ch, asic, 1],$
	 ...
	 oplot,[peaks[xr,ch,asic,0],peaks[2:xr-1,ch,asic,0]],$
	     [polyfit[n_elements(polyfit)-1],polyfit[0:(n_elements(polyfit)-2)]],$
	     thick=4,color=120
	 ;oplot,[peaks[xr,ch,asic,0],peaks[2:xr-1,ch,asic,0]],polyfit(sort(polyfit)),thick=4,color=120
	 ;if (asic gt 1) then stop
	 ...
	 ;!p.multi=[0,2,4]
	 set_plot,'PS'
	 fname='nonlinearity_asic'+strtrim(string(asic),2)+'cr.eps'
	 device,filename=fname,/color
	 ...
	 ;plot,nl[1,yr,asic],ytitle='NL',title='7.46kev',psym=10
 ...
 for asic=0,3 do begin
	  ...
	  !p.multi=[0,2,2]
	  set_plot,'PS'
	  fname='Gaincal_quadratio_asic'+strtrim(string(asic),2)+'cr.eps'
	  device,filename=fname,/color
      endif
         if (asic le 1) then tit = 'n-side (ASIC '+strtrim(string(asic),2)+')' else tit='p-side (ASIC '+strtrim(string(asic),2)+')'
...
set_plot,'x'
 ...
 !p.multi=[0,2,2]
     set_plot,'ps'
     fname='quadratio_histogram_cr.eps'
 ...
 !p.multi=[0,2,2]
     set_plot,'ps'
     fname='fe_quadratio_histogram_cr.eps'
     device,filename=fname,/encapsulated
    ...
    !p.multi=[0,2,2]
     set_plot,'ps'
     fname='ni_quadratio_histogram_cr.eps'
     device,filename=fname,/encapsulated
   ...
   !p.multi=[0,2,2]
     set_plot,'ps'
     fname='cu_quadratio_histogram_cr.eps'
     device,filename=fname,/encapsulated
  ...
  !p.multi=[0,2,2]
     set_plot,'ps'
     fname='am_quadratio_histogram_cr.eps'
     device,filename=fname,/encapsulated
     ...
     set_plot,'x'
	  ...
	  !X.MARGIN=!X.MARGIN/0.9
	  !Y.MARGIN=!Y.MARGIN/0.95
  !p.multi=[0,2,2]
     set_plot,'ps'
     fname='cr_quadratio_histogram_cr.eps'
     device,filename=fname,/encapsulated
      ...
      if (asic le 1) then title = 'n-side' else title='p-side'
      if((asic eq 0) or (asic eq 2)) then $
	 cghistoplot,quad_ratio[xr-2,0:61,asic],thick=4,title=title,xthick=4,ythick=4,charthick=3,charsize=1.25,color='blue',font=1,yticks=4 $
     else $
     cghistoplot,quad_ratio[xr-2,2:63,asic],thick=4,title=title,charsize=1.25,charthick=3,xthick=4,ythick=4,color='blue',font=1,yticks=4
     legend,'ASIC '+ strtrim(string(asic),2),box=0,thick=3,charthick=3,charsize=1.2,font=1,/right
     legend,'5.0 keV',box=0,thick=3,charthick=3,charsize=1.2,font=1,/left
     cgtext,0.5,0.01,'Model/Data',/normal,$
		 font=1,orientation=0,charsize=1.52,charthick=3
...
;!p.multi=[0,2,2]
 ...
 ;for asic=0,3 do begin
      ;if asic gt 1 then xr=10 else xr=8
     ;if((asic eq 0) or (asic eq 2)) then $
	 ;cghistoplot,quad_ratio[0:xr-2,0:61,asic],thick=2,charsize=0.8 $
     ;else cghistoplot,quad_ratio[0:xr-2,2:63,asic],thick=2,charsize=0.8
     ;legend,'ASIC '+ strtrim(string(asic),2),box=0,thick=2,charsize=0.8
 ;endfor
     ;device,/close
     ;set_plot,'x'

 stop
 save,quadfit_par,file='quadfit_par_cr.sav'
 stop
END
