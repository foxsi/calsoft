...
;                 stop:0, $          ; stop bit, should be 1
  ...
  for j = 0, n_files-1 do begin
    ...
    for i = long(0), n_evts-1 do begin
      ...
      temp_struct[i].ped= raw_data.common_mode[i]
      ...
      temp_struct[i].common_mode        = raw_data.pedestal[i]
...
endwhile
  ...
  for i = long(0), n_evts-1 do begin
      if (temp_struct[i].asic eq 5) then temp_struct[i].error_flag = 1
      if temp_struct[i].asic eq 1 and temp_struct[i].sync ne 'eb91' then temp_struct[i].error_flag = 1
      if temp_struct[i].asic eq 2 and temp_struct[i].sync ne 'eb92' then temp_struct[i].error_flag = 1
      if temp_struct[i].asic eq 3 and temp_struct[i].sync ne 'eb93' then temp_struct[i].error_flag = 1
